...
export default function CreateRecord() {
  ...
  const handleCreate = async (e) => {
    ...
    if (uploadMode === 'form' && selectedOps.length === 0) {
      setToast({ message: 'At least one operation is required', type: 'error' });
      return;
    }
    ...
    try {
      ...
      const { data: pkData } = await API.get(`/auth/public-key/${patientAddress}`);
      const patientNaClPublicKey = pkData.user.naclPublicKey;
      if (!patientNaClPublicKey) throw new Error('Patient has no NaCl public key registered');

      // 2. Build tags
      const allTags = [
        ...selectedTags,
        ...customTags.split(',').map((t) => t.trim().toLowerCase()).filter(Boolean),
      ];
      ...
      if (!data.success || !data.cid) throw new Error(data.error || 'Upload failed');
      ...
      setToast({ message: `Record created! CID: ${data.cid.slice(0, 12)}…`, type: 'success' });

      // Reset
      setPatientAddress(''); setPatientName(''); setPatientAge(''); setPatientGender('');
      setBloodGroup(''); setAllergies(''); setChiefComplaint(''); setSymptoms('');
      setDiagnosis(''); setTreatment(''); setPrescription(''); setLabResults('');
      setFollowUp(''); setNotes(''); setBp(''); setPulse(''); setTemperature('');
      setWeight(''); setSelectedOps([]); setSpecialty(''); setCustomTags(''); setPdfFile(null);
    } catch (err) {
      setToast({ message: err?.response?.data?.error || err?.reason || err?.message || 'Failed', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-6 animate-fade-in">
      ...
      <div>
        ...
        <p className="text-text-secondary text-sm mt-1">All fields are encrypted before leaving your browser</p>
      </div>
      ...
      <form onSubmit={handleCreate} className="space-y-6">
        ...
        <Card>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              <Input id="cr-addr" label="Patient Ethereum Address *" placeholder="0x…"
                value={patientAddress} onChange={(e) => setPatientAddress(e.target.value)} required />
            </div>
            <Input id="cr-name" label="Patient Full Name" placeholder="e.g. Aisha Khan"
              value={patientName} onChange={(e) => setPatientName(e.target.value)} />
            <Input id="cr-age" label="Age" type="number" placeholder="e.g. 34"
            ...
            <div>
              ...
              <select id="cr-gender" value={patientGender} onChange={(e) => setPatientGender(e.target.value)}
            ...
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-1.5">Blood Group</label>
              <select id="cr-blood" value={bloodGroup} onChange={(e) => setBloodGroup(e.target.value)}
                className="w-full px-4 py-2.5 rounded-xl bg-surface-input border border-border text-text-primary focus:outline-none focus:ring-2 focus:ring-primary/40">
                <option value="">Select…</option>
            ...
            <div className="sm:col-span-2">
              <Input id="cr-allergy" label="Known Allergies" placeholder="e.g. Penicillin, Shellfish (or 'None')"
        ...
        {uploadMode === 'form' && (
        <Card>
          <h2 className="text-lg font-semibold text-text-primary mb-4">Vital Signs</h2>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <Input id="cr-bp"   label="Blood Pressure" placeholder="120/80 mmHg"
              value={bp}          onChange={(e) => setBp(e.target.value)} />
            <Input id="cr-hr"   label="Heart Rate (bpm)" placeholder="72"
              value={pulse}       onChange={(e) => setPulse(e.target.value)} />
            <Input id="cr-temp" label="Temperature (°C)" placeholder="37.0"
              value={temperature} onChange={(e) => setTemperature(e.target.value)} />
            <Input id="cr-wt"   label="Weight (kg)" placeholder="65"
        ...
        <Card>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <select id="cr-type" value={recordType} onChange={(e) => setRecordType(e.target.value)}
                  className="w-full px-4 py-2.5 rounded-xl bg-surface-input border border-border text-text-primary focus:outline-none focus:ring-2 focus:ring-primary/40">
              ...
              <div>
                ...
                <select id="cr-specialty" value={specialty} onChange={(e) => setSpecialty(e.target.value)}
                  className="w-full px-4 py-2.5 rounded-xl bg-surface-input border border-border text-text-primary focus:outline-none focus:ring-2 focus:ring-primary/40">
        ...
        {uploadMode === 'form' && (
        <Card>
          <h2 className="text-lg font-semibold text-text-primary mb-4">Clinical Details</h2>
          <div className="space-y-4">
            <Input id="cr-cc" label="Chief Complaint" placeholder="e.g. Chest pain since 2 days"
              value={chiefComplaint} onChange={(e) => setChiefComplaint(e.target.value)} />

            <div>
              <label className="block text-sm font-medium text-text-secondary mb-1.5">Symptoms</label>
              <textarea id="cr-sym" rows={2} placeholder="e.g. Fatigue, excessive thirst, frequent urination…"
            ...
            <Input id="cr-diag" label="Diagnosis" placeholder="e.g. Type-2 Diabetes Mellitus"
              value={diagnosis} onChange={(e) => setDiagnosis(e.target.value)} />

            <Input id="cr-treat" label="Treatment Plan" placeholder="e.g. Metformin 500mg twice daily"
              value={treatment} onChange={(e) => setTreatment(e.target.value)} />

            <Input id="cr-rx" label="Prescription" placeholder="e.g. Metformin 500mg, Paracetamol 650mg"
              value={prescription} onChange={(e) => setPrescription(e.target.value)} />

            <div>
              <label className="block text-sm font-medium text-text-secondary mb-1.5">Lab Results</label>
              <textarea id="cr-lab" rows={2} placeholder="e.g. HbA1c: 8.2%, FBS: 180 mg/dL…"
                value={labResults} onChange={(e) => setLabResults(e.target.value)}
                className="w-full px-4 py-2.5 rounded-xl bg-surface-input border border-border text-text-primary focus:outline-none focus:ring-2 focus:ring-primary/40 resize-none" />
            </div>

            <Input id="cr-fu" label="Follow-Up Plan" placeholder="e.g. Review in 4 weeks, repeat HbA1c"
            ...
            <div>
              ...
              <textarea id="cr-notes" rows={2} placeholder="Any other clinical observations…"
        ...
        {/* ─── Operations / Tags ─── */}
        <Card>
          <h2 className="text-lg font-semibold text-text-primary mb-4">Operations & Tags *</h2>
          <div className="flex flex-wrap gap-2 mb-3">
            {OPERATIONS.map((op) => {
              const active = selectedOps.includes(op.value);
              return (
                <button key={op.value} type="button" onClick={() => toggleOp(op.value)}
                  ...
                  {active && <X className="w-3 h-3 inline mr-1" />}{op.label}
                </button>
              );
            })}
          </div>
          {selectedTags.length > 0 && (
            <div className="mb-3">
              ...
              {selectedTags.map((t) => (
                <span key={t} className="inline-block px-2 py-0.5 mr-1 mb-1 rounded-md bg-primary/10 text-primary text-xs">{t}</span>
              ))}
            </div>
          )}
          <Input id="cr-tags" label="Extra Tags (comma separated)" placeholder="HbA1c, fasting, followup"

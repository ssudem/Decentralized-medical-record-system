...
export default function PatientDashboard() {
  ...
  const handleApproveRequest = async (request) => {
    ...
    try {
      ...
      const { data: doctorData } = await API.get(
        `/auth/public-key/${request.doctor_address}`,
      );
      ...
      for (const rec of records) {
        try {
          ...
          keysFailed++;
          console.warn(
            `[ApproveRequest] Key re-wrap failed for CID ${rec.cid}:`,
            grantErr.message,
          );
        }
      }
  ...
  const opLabel = (val) =>
    OPERATIONS.find((o) => o.value === val)?.label || val;
              ...
              >
                ...
                <div className="mb-4">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleDurationChange(req.id, e.target.value)
                    }

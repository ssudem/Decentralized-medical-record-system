...
export default function GrantAccess() {
  ...
  const handleGrant = async (e) => {
    ...
    if (!doctorAddress || !operation || selectedCIDs.length === 0) {
      setToast({ message: 'Select doctor, operation, and at least one record', type: 'error' });
      return;
    }
    if (!naclPrivateKey) {
      setToast({ message: 'Encryption keys not available. Re-login.', type: 'error' });
      return;
    }
    ...
    try {
      ...
      const { data: pkData } = await API.get(`/auth/public-key/${doctorAddress}`);
      ...
      setToast({ message: 'Confirm blockchain transaction in MetaMask…', type: 'info' });
      const durationSeconds = parseInt(hours) * 3600;
      await grantAccessOnChain(doctorAddress, operation, `Manual grant: ${operation}`, durationSeconds);
      ...
      setToast({ message: `Access granted to ${doctorAddress.slice(0, 8)}… for ${hours}h`, type: 'success' });
      setDoctorAddress('');
      setSelectedCIDs([]);
    } catch (err) {
      const msg = err?.response?.data?.error || err?.reason || err?.message || 'Grant failed';
      ...
      <form onSubmit={handleGrant} className="space-y-6">
        ...
        <Card>
          ...
          <div className="space-y-4">
            <Input id="ga-doc" label="Doctor Ethereum Address" placeholder="0x…"
              value={doctorAddress} onChange={(e) => setDoctorAddress(e.target.value)} required />

            <div>
              <label htmlFor="ga-op" className="block text-sm font-medium text-text-secondary mb-1.5">Operation</label>
              <select id="ga-op" value={operation} onChange={(e) => setOperation(e.target.value)} required
            ...
            <div>
              ...
              <Input id="ga-hours" type="number" min="1" max="168" placeholder="1"
        ...
        <Card>
          ...
          {recLoading ? (
            ...
            <p className="text-text-muted text-sm text-center py-4">No records found for this operation.</p>
          ) : (
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {records.map((rec) => (
                <label key={rec.cid}
                  ...
                  <input type="checkbox" checked={selectedCIDs.includes(rec.cid)}
                    onChange={() => toggleCID(rec.cid)}
                    className="rounded text-primary" />
                  <div className="flex-1 min-w-0">
                    <span className="text-xs font-mono text-text-secondary break-all">{rec.cid}</span>
        ...
        <Button type="submit" loading={loading} className="w-full">
          ...
          Grant Access ({selectedCIDs.length} record{selectedCIDs.length !== 1 ? 's' : ''})
        </Button>
      </form>
    </div>
  );
